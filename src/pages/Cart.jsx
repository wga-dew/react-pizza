...
function Cart() {
  ...
  const addedPizzas = Object.keys(items).map((key, i) => {
  ...
  const onRemoveItem = (id) => {
    if(window.confirm(`Вы действительно хотите удалить пицу "${items[id].items[0].name}" из корзины?`)) {
  ...
  const onClickOrder = () => {
    console.log('ВАШ ЗАКАЗ', items, totalPrice + ' грн');
  };
